...
@section('contenido')
    ...
    <h3>Estas viendo la <b>{{ $nombreCompetencia->nombre }}</b> <span class="badge bg-secondary">Jueces Aceptados
            {{ count($juecesAceptados) . ' de ' . $nombreCompetencia->cantidadJueces }}</span>
        @if ($nombreCompetencia->estadoJueces == 1)
        <p class="badge bg-info">Inscripcion de jueces cerrada</p>
        @endif
    </h3>
                        ...
                        <td class="list-group my-auto">
                            @if ($row['estado'] == 0)
                                @if ($row->tieneSolicitud)
                                    <a href="{{ route('competidor_solicitudes', ['id' => $row->juez->id]) }}"
                                ...
                                @else
                                    <a href="{{ route('habilitar_juez', ['id' => $row->idCompetenciaJuez]) }}"
                                        class="btn btn-outline-success"><i
                                            class="bi bi-check2-square me-2"></i>Habilitar</a>
                                    <a href="{{ route('rechazar_juez', ['id' => $row->idCompetenciaJuez]) }}"
                                        class="btn btn-outline-danger"><i class="bi bi-x-circle me-2"></i>Rechazar</a>
                                @endif
                            @elseif($row['estado'] == 2)
                                <a href="{{ route('delete_juez_competencia', ['id' => $row->idCompetenciaJuez]) }}"
