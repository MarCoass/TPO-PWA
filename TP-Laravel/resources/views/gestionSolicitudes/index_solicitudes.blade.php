...
@section('scripts')
    <script>
        datatables("tabla_solicitud", 4, "asc")
    </script>
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          exampleModal.addEventListener('show.bs.modal', function(event) {
              ...
              const url = '{{ route('aceptar_solicitud', ['id' => ':idSolicitud']) }}'.replace(
                  ':idSolicitud', idSolicitud);
              const url2 = '{{ route('rechazar_solicitud', ['id' => ':idSolicitud']) }}'.replace(
                  ':idSolicitud', idSolicitud);
...
@section('contenido')
    ...
    @if (Route::currentRouteName() == 'solicitudes_archivadas')
        ...
        <div class="m-2">
            <a class="btn btn-primary" href="{{ route('index_solicitudes') }}"><i class="bi bi-bell-fill me-2"></i> Ver
                Solicitudes</a>
            <a class="btn btn-outline-danger" href="{{ route('borrarArchivados') }}"><i class="bi bi-trash me-2"></i>
                Eliminar todas</a>
        </div>
    @else
        <h3>Solicitudes.</h3>
        <div class="m-2">
            <a class="btn btn-primary mb-2" href="{{ route('solicitudes_archivadas') }}"><i
                    class="bi bi-eye-slash-fill me-2"></i> Ver Solicitudes Archivadas</a>
        </div>
    @endif
    <br>

    <table id="tabla_solicitud" class="table hover table-light table-bordered nowrap border dataTable dtr-inline collapsed"
        width="100%">
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($solicitud->estadoSolicitud == 6 || $solicitud->estadoSolicitud == 5)
                                <a href="{{ route('ocultar_solicitud', ['id' => $solicitud->idSolicitud]) }}"
                                    class="btn btn-outline-danger"><i class="bi bi-eye-slash-fill me-2"></i>ocultar</a>
                            @elseif ($solicitud->estadoSolicitud == 4)
                                <button id="verSolicitudBtn" type="button" class="btn btn-outline-success"
                                    data-bs-toggle="modal" data-bs-target="#exampleModal"
                                    data-id="{{ $solicitud->idSolicitud }}"><i class="bi bi-pencil-square me-2"></i>Ver
                                    solicitud</button>
                            @endif
    ...
    @if ($solicitud)
        <!-- Modal -->
        <div class="modal fade modalTemaDual" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content modalTemaDual">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">El usuario {{ $solicitud->user->usuario }} solicita
                        </h5>
                    ...
                    <div class="modal-body ">
                        ...
                        @if ($solicitud->newGraduacion != 0)
                            ...
                            <div>
                                ...
                                <p>de </p>
                                <b>{{ $competidores->firstWhere('idUser', $solicitud->idUser)->graduacion->nombre }} -
                                    {{ $competidores->firstWhere('idUser', $solicitud->idUser)->graduacion->color }}</b>
                                <p>a </p>
                                <b>{{ $graduaciones->firstWhere('idGraduacion', $solicitud->newGraduacion)->nombre }} -
                                    {{ $graduaciones->firstWhere('idGraduacion', $solicitud->newGraduacion)->color }}</b>
                            </div>
                        @endif
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                        <a id="aceptarSolicitudBtn" href="#" class="btn btn-outline-success"><i
                                class="bi bi-check2-square me-2"></i>Aceptar Solicitud</a>
                        <a id="rechazarSolicitudBtn" href="#" class="btn btn-outline-danger"><i
                                class="bi bi-x-circle me-2"></i>Rechazar Solicitud</a>
                    </div>
                </div>
            </div>
        </div>
