    ...
    public function actualizarDatosPersonales(Request $request)
    {
        ...
        if (password_verify($request->input('password'), $usuario->password)) {
            try {
                ...
                $usuario->save();
                $datosSolicitud[0] = " Se ha actualizado su nombre a ".$usuario->nombre." ".$usuario->apellido." si usted no ha efectuado estos cambios contactese con soporte para reportar el hecho. ";
